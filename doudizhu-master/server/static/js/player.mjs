...
export class Player {
    ...
    showError(msg) {
        ...
        const text = this.game.add.text(width / 2, height / 2 - 80, msg, {
        ...
        text.alpha = 0;
        // 淡入 → 停留 → 淡出
        this.game.add.tween(text).to({alpha: 1}, 200, Phaser.Easing.Default, true);
        this.game.time.events.add(1500, () => {
            this.game.add.tween(text).to({alpha: 0}, 400, Phaser.Easing.Default, true)
                .onComplete.add(() => text.destroy());
        });
    }
    ...
    canPlay(lastTurnPoker, shotPoker) {
        // 检查特殊牌（101-111）是否被错误地拆分或混合
        let hasSpecial = shotPoker.some(id => Poker.isSpecialId(id));
        if (hasSpecial) {
            let allSpecial = shotPoker.every(id => Poker.isSpecialId(id));
            if (!allSpecial) {
                return '特殊牌不能和普通牌混合出';
            }
            let zmjjkkCount = shotPoker.filter(id => id >= 101 && id <= 106).length;
            let kskblCount = shotPoker.filter(id => id >= 107 && id <= 111).length;
            let zdjdCount = shotPoker.filter(id => id >= 112 && id <= 115).length;
            if (zmjjkkCount > 0 && zmjjkkCount < 6) {
                return 'zmjjkk必须6张一起出';
            }
            if (kskblCount > 0 && kskblCount < 5) {
                return 'kskbl必须5张一起出';
            }
            if (zdjdCount > 0 && zdjdCount < 4) {
                return 'zdjd必须4张一起出';
            }
            if ((zmjjkkCount > 0 && kskblCount > 0) ||
                (zmjjkkCount > 0 && zdjdCount > 0) ||
                (kskblCount > 0 && zdjdCount > 0)) {
                return '不同特殊牌组不能混在一起出';
            }
        }
        ...
        // 双方都是普通牌（<30000）时要求类型一致；炸弹/火箭/特殊牌（≥30000）可直接比大小
        if (valueA[0] !== valueB[0] && valueA[1] < 30000 && valueB[1] < 30000) {
            return '出牌类型跟上家不一致';
        }
    ...
    reDealPoker() {
        this.sortPoker();
        let length = this.pokerInHand.length;
        let gap = Math.min(this.game.world.width / length, Poker.PW * 0.44);
        for (let i = 0; i < length; i++) {
            let pid = this.pokerInHand[i];
            let p = this.findAPoker(pid);
            if (p) {
                p.bringToTop();
                p.x = this.game.world.width / 2 + (i - (length - 1) / 2) * gap;
    ...
    pokerSelected(pokers) {
        for (let i = 0; i < pokers.length; i++) {
            ...
            if (p) p.y = this.game.world.height - Poker.PH * 0.8;
        }
    }
    ...
    pokerUnSelected(pokers) {
        for (let i = 0; i < pokers.length; i++) {
            let p = this.findAPoker(pokers[i]);
            if (p) p.y = this.game.world.height - Poker.PH / 2;
        }
    }
}
...
export class NetPlayer extends Player {
    ...
    _buildPokerSprites() {
        ...
        const length = this.pokerInHand.length;
        for (let i = 0; i < length; i++) {
            // Use id=0 (≤54) to hit spritesheet branch, frame=55 for card back (frame 54)
            const p = new Poker(this.game, 0, 55);
            this.game.world.add(p);
            this.pushAPoker(p);
        }
    }
    ...
    dealPoker() {
        let length = this.pokerInHand.length;
        for (let i = 0; i < length; i++) {
            // Always use card back for robot hand
            const p = new Poker(this.game, 0, 55);
            this.game.world.add(p);
            this.pushAPoker(p);
            this.dealPokerAnim(p, i);
    ...
    arrangePoker() {
        if (this.pokerInHand.length > 0) {
            // Check if any normal card exists (pokerInHand is sorted, special cards come first)
            const hasNormal = this.pokerInHand.some(id => id <= 54);
            if (hasNormal) {
                this.reDealPoker();
            }
        }
    ...
    reDealPoker() {
        this.sortPoker();
        const length = this.pokerInHand.length;
        // All hand sprites have id=0 (card backs), iterate _pokerPic by index
        for (let i = 0; i < length; i++) {
            const p = this._pokerPic[i];
            if (p) {
                p.bringToTop();
    ...
    cleanPokers() {
        // Kill all sprites in _pokerPic (they all have id=0)
        for (let i = 0; i < this._pokerPic.length; i++) {
            const p = this._pokerPic[i];
            if (p) p.kill();
        }
        this.pokerInHand = [];
    ...
    dealPokerAnim(p, i) {
        const width = this.game.world.width;
        // id=0 (card back) or >53 (joker/special) → stack together
        if (p.id > 53 || p.id === 0) {
            this.game.add.tween(p).to({
